// c6e20calculatorWithFunctions.cpp: определяет точку входа для консольного приложения.
//
...
double prim(bool get)
{
	...
	case NUMBER:
	{
		double v = y.number_value;
		get_token();
		return v;
	}
	case NAME:
	{
		double&v = table[y.string_value];
...
Token_value get_token()
{
	...
	default:
		...
		{
			y.string_value = ch;
			while (istr.get(ch) && isalnum(ch)) y.string_value.push_back(ch);
			istr.putback(ch);

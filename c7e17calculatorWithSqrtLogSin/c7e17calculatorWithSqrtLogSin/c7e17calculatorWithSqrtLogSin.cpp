// c7e17calculatorWithSqrtLogSin.cpp: определяет точку входа для консольного приложения.
//
...
string subprocessor(string);
string subprocessor(string stri0)
{

	smatch f00;
	while (regex_search(stri0, f00, regex("([\\.0-9-]+[\\*/\\+\\-][\\.0-9-]+|\\W\\([\\.0-9-]+\\))")))
	{
		//cout << "iterations\n";
		while (regex_search(stri0, f00, regex("([\\.0-9-]+)\\*([\\.0-9-]+)")))
		{
			//cout << "*\n";
			stri0 = regex_replace(stri0, regex("([\\.0-9-]+)\\*([\\.0-9-]+)"), d_to_s(s_to_d(f00[1]) * s_to_d(f00[2])), regex_constants::format_first_only);
			//cout << "*\n";
		}
		while (regex_search(stri0, f00, regex("([\\.0-9-]+)/([\\.0-9-]+)")))
		{
		...
		while (regex_search(stri0, f00, regex("(\\W)\\(([\\.0-9-]+)\\)")))
		{
			stri0 = regex_replace(stri0, regex("(\\W)\\(([\\.0-9-]+)\\)"), string(f00[1])+ string(f00[2]), regex_constants::format_first_only);
		}
	}
	if (regex_search(stri0, f00, regex("([\\.0-9-]+[\\*/\\+\\-][\\.0-9-]+|\\W\\([\\.0-9-]+\\))")))
		stri0=subprocessor(stri0);
	return stri0;
}
...
bool preprocessor()
{
	...
	//cout << "0 s\n";
	stri = subprocessor(stri);
	smatch f0;
	while (regex_search(stri, f0, regex("sqrt\\(([\\.0-9-]+)\\)")))
	{
		if (s_to_d(f0[1]) < 0)
		{
			error("you mast input sqrt of positive number");
			return true;
		}
		stri = regex_replace(stri, regex("sqrt\\(([\\.0-9-]+)\\)"), d_to_s(sqrt(s_to_d(f0[1]))), regex_constants::format_first_only);
	}
	while (regex_search(stri, f0, regex("log\\(([\\.0-9-]+)\\)")))
	{
		if (s_to_d(f0[1]) < 0)
		{
			error("you mast input log of positive number");
			return true;
		}
		stri = regex_replace(stri, regex("log\\(([\\.0-9-]+)\\)"), d_to_s(log(s_to_d(f0[1]))), regex_constants::format_first_only);
	}
	while (regex_search(stri, f0, regex("sin\\(([\\.0-9-]+)\\)")))
	{
		stri = regex_replace(stri, regex("sin\\(([\\.0-9-]+)\\)"), d_to_s(sin(s_to_d(f0[1]))), regex_constants::format_first_only);
	}
	smatch f1;
	while (regex_search(stri, f1, regex("([[:alpha:]]\\w*)\\(((([\\.0-9-]+),)*([\\.0-9-]+))\\)")))
	{
		...
		for (unsigned i = 0; i < v_variables.size(); i++)
			variables[functions[name_of_function][i + 1]] = s_to_d(v_variables[i]);

		stri = regex_replace(stri, regex("[[:alpha:]]\\w*\\((([\\.0-9-]+),)*([\\.0-9-]+)\\)"), "(" + functions[name_of_function][0] + ")", regex_constants::format_first_only);
		cout << stri << '\n';
...
int main()
{
	...
	//vector<string> test={ "!f(x,y)=x+y","!f(1,2)" };
	//int jjj = 0;
